...
export default function ChatWindow({ currentUser, selectedUser, onViewProfile, onBack }: ChatWindowProps) {
  ...
  const handleSwipeMove = (e: React.TouchEvent, messageId: string) => {
    ...
    // Only allow swipe to the left (negative deltaX) up to 100px
  ...
  const handleSwipeEnd = (e: React.TouchEvent, message: Message) => {
    ...
    // If swiped more than 60px to the left, trigger reply
    if (deltaX < -60) {
      ...
      <form onSubmit={sendMessage} className="border-t border-amber-200 dark:border-red-900 bg-gradient-to-r from-amber-50 to-red-50 dark:from-gray-900 dark:to-red-950 shadow-lg overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 px-0 py-2">
          ...
          <input
            ...
            placeholder="Type..."
            disabled={loading}
            className="flex-1 min-w-0 max-w-[65%] px-3 py-2 text-sm border-2 border-amber-300 dark:border-red-800 rounded-full focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white disabled:opacity-50 placeholder-red-400 dark:placeholder-yellow-600 mx-2"
          />
